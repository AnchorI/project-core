...
if (config.env !== Env.Test) {
    sequelize
        .sync({ alter: false })
        .then(() => {
            logger.info(`The database is connected. ${database[config.env]?.host || database.default.host}`)
        })
        .catch((error: Error) => {
    ...
    sequelize.query = function (sql: Sql, options?: SqlOptions) {
        return SequelizeLogger.prototype.query.apply(this, [sql, options]).catch(error => {
            if (config.env === Env.Production) {
                // sentry.handleError(`Sequelize Error: ${error.message}. Raw query: ${sql}`)
            }

            logger.error(`Sequelize Error: ${error.message}. Raw query: ${sql}`)
        })
    }
}
